...
var creditText:FlxText = new FlxText(876, 648, 348);
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (player < 1)
				targetAlpha = ClientPrefs.opponentArrowOpacity;
			else if (player == 1)
				targetAlpha = targetAlpha = ClientPrefs.arrowOpacity;
...
//              }
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (!ClientPrefs.sustainhp && note.isSustainNote)
				health -= note.hitHealth * healthGain;
	...
	override function beatHit()
	{
		...
		if (curBeat % gfSpeed == 0
			...
			} // i dont understand why tf this works but ok
		{
			gf.dance();
		}
